        ...
        for(int index_counter = 0; index_counter < num_filhos && pattern_found != 1 && no_more_sons_to_search != 1;index_counter++){
            ...
            if (temp_root->filhos[index_counter] != NULL) {
                ...
                if (son_data == char_to_search){
                    //printf("Test2\n");
                    //printf("Parte_ou_filho_encontrado->%c\n",son_data);
                    //printf("test->%d\n",temp_root->indice);
...
void lzwd(struct trie_node* root,unsigned char* bloco, int dic_limit,struct statistics *statistics_store, FILE* outputfile, int dic_mode, int max_size){
    ...
    memcpy(Pa, bloco,1);
    //int max_size = getsize_of_array(bloco);
    //max_size = t;
    int block_end = 0;
    //printf("Block->%s\n",bloco);
    //printf("test_attemp on lzwd->%d\n",test_attempt);
    //printf("test_attemp on lzwd with add->%d\n",test_attempt);
    //printf("Max_size -> %d\n",max_size);
    ...
    for(i = 0; i<max_size && block_end == 0 && max_size > 2; i++){
        ...
        if(primeira_it == 1){
            ...
            //temp = procurar_na_trie_total(temp,Pa,Pa_size);
        ...
        while(filho_pb_encontrado != -1 && block_end != 1){
            ...
            //printf("Hello2\n");
            if(pos_avancar!=1){
                //printf("pos_avancar val->%d\n",pos_avancar);
                //printf("Hello3\n");
                Pb[pos_avancar]='\0';
                //printf("pos_avançar->%d\n",pos_avancar);
                //sleep(1);
            ...
            if(temp_Pb != NULL){
                ...
                temp_final = temp_Pb;
                Pb_Pa_size = Pb_size;
                //Pb_final = (unsigned char*)realloc(Pb_final, (tamanho_Pb)*sizeof(unsigned char)); 
                //printf("Hello4\n");
            ...
            if(filho_pb_encontrado != -1){
                ...
                //pattern_to_dictionary = (unsigned char*)realloc(pattern_to_dictionary, ((tamanho_Pa+1)+tamanho_Pb-1));
                //pattern_to_dictionary[((tamanho_Pa)+(tamanho_Pb))] = '\0';
                ...
                if(NP_A_PROCURAR != NULL){
                    ...
                    if(statistics_store->total_new_patterns <= dic_limit){
                        ...
                        //printf("value:%d | limit:%d\n",statistics_store->total_patterns_added,dic_limit);
                    }else{
                        //printf("FULL DIC\n");
                    }
                    //  inserir(root,pattern_to_dictionary,temp->indice,outputfile);
        ...
        //printf("Hello7\n");
        //Pa = (unsigned char*)realloc(Pa, ((Pb_Pa_size+1)*sizeof(unsigned char)));
        //printf("Hello8\n");
...
int main(int argc, char *argv[]) {
    ...
    //End of report
    long tamanho_ultimo;
    int nblocos = fsize(file,NULL,&block_size,&tamanho_ultimo);
    int tamanho_total = nblocos*block_size+tamanho_ultimo;
    if(dic_mode != 1){
    ...
    printf("Tamanho em bytes do ficheiro comprimido: %ld\n", total_num_of_bits / 8);
